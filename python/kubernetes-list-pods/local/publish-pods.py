...
parser.add_argument('--output-pods', help="output all pods to stdout", action="store_true", dest='enable_output')
...
async def run(loop):
    ...
    async def get_pods():
        ...
        for pod in w.stream(v1.list_pod_for_all_namespaces):
            logger.info("Pod: %s %s %s IP: %s" % (pod['object'].metadata.namespace, pod['object'].kind, pod['object'].metadata.name, pod['object'].status.pod_ip))
            msg = {'type':pod['type'],'object':pod['raw_object']}
            if args.enable_output:
                print(json.dumps(msg))

            await nc.publish("k8s_pods", json.dumps(msg).encode('utf-8'))
