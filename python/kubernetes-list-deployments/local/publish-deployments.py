...
async def run(loop):
    ...
    async def get_deployments():
        w = watch.Watch()
        for item in w.stream(api_instance.list_deployment_for_all_namespaces):
            logger.info("Deployment: %s %s Replicas:%s" % (item['object'].metadata.namespace, item['object'].metadata.name, item['object'].spec.replicas))
            msg = {'type':item['type'],'object':item['raw_object']}
            if args.enable_output:
                print(json.dumps(msg))

            await nc.publish("k8s_deployments", json.dumps(msg).encode('utf-8'))
